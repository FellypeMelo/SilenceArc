...
// Helper to cast pointers
#define DX_DEVICE ((ID3D11Device*)device_)
#define DX_CONTEXT ((ID3D11DeviceContext*)device_context_)
#define DX_SWAPCHAIN ((IDXGISwapChain*)swap_chain_)
#define DX_RTV ((ID3D11RenderTargetView*)render_target_view_)
#define WIN_HWND ((HWND)hwnd_)

static LRESULT WINAPI WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    if (ImGui_ImplWin32_WndProcHandler(hWnd, msg, wParam, lParam))
        return true;

    UIManager* manager = (UIManager*)GetWindowLongPtr(hWnd, GWLP_USERDATA);

    switch (msg) {
    case WM_SIZE:
        if (wParam == SIZE_MINIMIZED) {
            if (manager) manager->ShowWindow(false);
            return 0;
        }
        break;
    case WM_SYSCOMMAND:
    ...
    case WM_TRAYICON:
        if (LOWORD(lParam) == WM_LBUTTONUP) {
            if (manager) manager->ShowWindow(true);
...
bool UIManager::Init(const std::string& window_title, int width, int height) {
    ...
    SetWindowLongPtr(WIN_HWND, GWLP_USERDATA, (LONG_PTR)this);
...
void UIManager::ShowWindow(bool show) {
    if (!hwnd_) return;
    if (show) {
        ::ShowWindow(WIN_HWND, SW_RESTORE);
...
void UIManager::CleanupDeviceD3D() {
    CleanupRenderTarget();
    if (swap_chain_) { DX_SWAPCHAIN->Release(); swap_chain_ = nullptr; }
    if (device_context_) { DX_CONTEXT->Release(); device_context_ = nullptr; }
    if (device_) { DX_DEVICE->Release(); device_ = nullptr; }
}

void UIManager::CreateRenderTarget() {
    ID3D11Texture2D* pBackBuffer;
    DX_SWAPCHAIN->GetBuffer(0, IID_PPV_ARGS(&pBackBuffer));
    ID3D11RenderTargetView* rtv = nullptr;
    DX_DEVICE->CreateRenderTargetView(pBackBuffer, nullptr, &rtv);
    render_target_view_ = rtv;
    pBackBuffer->Release();
}

void UIManager::CleanupRenderTarget() {
    if (render_target_view_) { DX_RTV->Release(); render_target_view_ = nullptr; }
}
...
void UIManager::SetTransparency(float alpha) {
    ...
    if (hwnd_) {
        ...
        if (alpha < 1.0f) {
            ::SetWindowLong(WIN_HWND, GWL_EXSTYLE, ::GetWindowLong(WIN_HWND, GWL_EXSTYLE) | WS_EX_LAYERED);
            ::SetLayeredWindowAttributes(WIN_HWND, 0, (BYTE)(255 * alpha), LWA_ALPHA);
        } else {
            ::SetWindowLong(WIN_HWND, GWL_EXSTYLE, ::GetWindowLong(WIN_HWND, GWL_EXSTYLE) & ~WS_EX_LAYERED);
        }
    }
...
void UIManager::EndFrame() {
    ...
    DX_CONTEXT->OMSetRenderTargets(1, (ID3D11RenderTargetView**)&render_target_view_, nullptr);
    DX_CONTEXT->ClearRenderTargetView(DX_RTV, clear_color_with_alpha);
