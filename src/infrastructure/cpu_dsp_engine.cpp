...
void CpuDspEngine::synthesize(const std::complex<float>* freq_in, float* output_hop) {
    ...
    for (size_t n = 0; n < m_fft_size; ++n) {
        ...
        for (size_t k = 0; k < m_freq_size_truncated; ++k) {
            ...
            float b_im = -m_dft_im[k * m_fft_size + n];

            val += factor * (f.real() * b_re - f.imag() * b_im);
        }
        reconstructed[n] = val * scale * m_window_cpu[n];
    }

    // Overlap-Add with the synthesis buffer
    for (size_t i = 0; i < m_hop_size; ++i) {
        output_hop[i] = std::clamp(reconstructed[i] + m_synthesis_buf[i], -1.0f, 1.0f);
    }

    // Update synthesis buffer for the next frame
    for (size_t i = 0; i < m_hop_size; ++i) {
        m_synthesis_buf[i] = reconstructed[i + m_hop_size];
    }
}
} // namespace sa::infrastructure::directml_impl
